...
function SectionTag({ children }) {
  return <p className="mb-3 text-sm font-bold uppercase tracking-[0.22em] text-[#D9A441]">{children}</p>;
}

function Cta({ children, href, secondary = false }) {
  return (
    <a href={href} target="_blank" rel="noreferrer" className={`inline-flex items-center justify-center gap-2 rounded-2xl px-6 py-4 text-sm font-bold transition hover:scale-[1.02] ${secondary ? "border border-white/15 text-white hover:bg-white/10" : "bg-[#D9A441] text-[#090B12] shadow-lg shadow-[#D9A441]/20 hover:bg-[#F1C45F]"}`}>
      {children}
    </a>
  );
}
...
export default function App() {
  ...
  return (
    <main id="inicio" className="min-h-screen bg-[#090B12] text-white">
      <header className="sticky top-0 z-50 border-b border-white/10 bg-[#090B12]/90 backdrop-blur-xl">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 lg:px-8">
          <a href="#inicio" className="flex items-center gap-3">
            <img src="/gabriel-profile.jpg" alt="Gabriel Castellanos" className="h-12 w-12 rounded-2xl object-cover ring-2 ring-[#D9A441]" />
            <div><p className="text-sm font-bold leading-tight">Gabriel Castellanos</p><p className="text-xs text-slate-400">{t.brandSub}</p></div>
          </a>
          ...
          <div className="hidden items-center gap-3 md:flex">
            ...
            <a href={whatsappUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-bold text-[#090B12] transition hover:bg-[#D9A441]">{t.primaryCta} <ArrowRight size={16} /></a>
          </div>
        ...
        {menuOpen && <div className="border-t border-white/10 px-6 py-4 md:hidden"><div className="flex flex-col gap-4">
          ...
          <a href={whatsappUrl} target="_blank" rel="noreferrer" className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#D9A441] px-5 py-3 text-sm font-bold text-[#090B12]">{t.primaryCta} <ArrowRight size={16} /></a>
        </div></div>}
      </header>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-6 py-20 md:grid-cols-2 lg:px-8 lg:py-28">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-[#D9A441]/40 bg-[#D9A441]/10 px-4 py-2 text-sm font-semibold text-[#F5D28E]"><ShieldCheck size={16} /> {t.heroTag}</div>
            ...
            <div className="mt-8 grid max-w-2xl gap-3 sm:grid-cols-3">{t.stats.map(([value, label]) => <div key={value} className="rounded-2xl border border-white/10 bg-white/[0.05] p-4"><p className="text-xl font-black text-[#D9A441]">{value}</p><p className="mt-1 text-xs text-slate-400">{label}</p></div>)}</div>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.7, delay: 0.12 }}>
            <div className="relative rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl backdrop-blur">
              <img src="/gabriel-profile.jpg" alt="Gabriel Castellanos" className="h-[430px] w-full rounded-[1.5rem] object-cover object-top opacity-95" />
              <div className="absolute inset-x-5 bottom-5 rounded-b-[1.5rem] bg-gradient-to-t from-[#090B12] via-[#090B12]/90 to-transparent p-6 pt-28"><p className="text-sm uppercase tracking-[0.22em] text-[#D9A441]">{t.cardTag}</p><h3 className="mt-3 text-3xl font-black">{t.cardTitle}</h3><p className="mt-3 leading-7 text-slate-300">{t.cardText}</p></div>
            </div>
          </motion.div>
        </div>
      </section>

      <section className="border-y border-white/10 bg-white/[0.04] py-6"><div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-x-10 gap-y-3 px-6 text-center text-sm font-semibold text-slate-300 lg:px-8">{t.strip.map((item, index) => <React.Fragment key={item}><span>{item}</span>{index < t.strip.length - 1 && <span className="hidden text-[#D9A441] sm:inline">•</span>}</React.Fragment>)}</div></section>

      <section className="bg-[#090B12] py-16">
        <div className="mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.04] shadow-2xl">
            <img
              ...
              alt={lang === "es" ? "Banner de marca Gabriel Castellanos" : "Gabriel Castellanos brand banner"}
      ...
      <section className="bg-white py-20 text-[#090B12]"><div className="mx-auto max-w-7xl px-6 lg:px-8"><div className="grid gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:items-start"><div><SectionTag>{t.forWhomTag}</SectionTag><h2 className="text-3xl font-black leading-tight md:text-5xl">{t.forWhomTitle}</h2><p className="mt-5 text-lg leading-8 text-slate-700">{t.forWhomText}</p></div><div className="grid gap-4 sm:grid-cols-2">{t.painPoints.map((item) => <div key={item} className="rounded-3xl border border-slate-200 bg-slate-50 p-5"><CheckCircle2 className="mb-4 text-[#B8862E]" size={24} /><p className="font-medium leading-7 text-slate-700">{item}</p></div>)}</div></div></div></section>

      <section id="sobre-mi" className="py-20"><div className="mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-center lg:px-8"><div className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-5 shadow-2xl"><img src="/gabriel-profile.jpg" alt="Foto profesional de Gabriel Castellanos" className="h-[520px] w-full rounded-[1.5rem] object-cover object-top" /><div className="mt-5 rounded-2xl bg-[#D9A441]/10 p-4 text-center"><p className="font-black text-[#F5D28E]">Estrategia · Procesos · Rentabilidad</p></div></div><div><SectionTag>{t.aboutTag}</SectionTag><h2 className="text-3xl font-black leading-tight md:text-5xl">{t.aboutTitle}</h2><p className="mt-6 text-lg leading-8 text-slate-300">{t.about1}</p><p className="mt-5 text-lg leading-8 text-slate-300">{t.about2}</p></div></div></section>

      <section className="bg-[#F6F0E6] py-20 text-[#090B12]"><div className="mx-auto max-w-7xl px-6 lg:px-8"><div className="mx-auto max-w-3xl text-center"><SectionTag>{t.areasTag}</SectionTag><h2 className="text-3xl font-black leading-tight md:text-5xl">{t.areasTitle}</h2></div><div className="mt-12 grid gap-6 md:grid-cols-3">{t.areas.map(([title, text, Icon]) => <div key={title} className="rounded-[1.75rem] bg-white p-6 shadow-xl"><div className="mb-5 inline-flex rounded-2xl bg-[#090B12] p-4 text-[#D9A441]"><Icon size={28} /></div><h3 className="text-xl font-black">{title}</h3><p className="mt-3 leading-7 text-slate-700">{text}</p></div>)}</div></div></section>

      <section id="sesion" className="py-20"><div className="mx-auto max-w-7xl px-6 lg:px-8"><div className="grid gap-10 lg:grid-cols-2 lg:items-center"><div><SectionTag>{t.sessionTag}</SectionTag><h2 className="text-3xl font-black leading-tight md:text-5xl">{t.sessionTitle}</h2><p className="mt-5 text-lg leading-8 text-slate-300">{t.sessionText}</p><div className="mt-8"><Cta href={whatsappUrl}>{t.primaryCta} <ArrowRight size={18} /></Cta></div></div><div className="grid gap-4 sm:grid-cols-2">{t.benefits.map(([title, text, Icon]) => <div key={title} className="rounded-[1.75rem] border border-white/10 bg-white/[0.06] p-6 shadow-xl"><div className="mb-5 inline-flex rounded-2xl bg-[#D9A441]/15 p-4 text-[#D9A441]"><Icon size={28} /></div><h3 className="text-xl font-black">{title}</h3><p className="mt-3 leading-7 text-slate-300">{text}</p></div>)}</div></div></div></section>

      <section className="bg-white py-20 text-[#090B12]"><div className="mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-2 lg:items-center lg:px-8"><div><SectionTag>{t.methodTag}</SectionTag><h2 className="text-3xl font-black leading-tight md:text-5xl">{t.methodTitle}</h2><p className="mt-5 text-lg leading-8 text-slate-700">{t.methodText}</p><div className="mt-8 space-y-5">{t.steps.map(([number, title, text]) => <div key={number} className="rounded-3xl bg-[#F6F0E6] p-6 shadow-xl"><div className="flex gap-5"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[#090B12] text-lg font-black text-[#D9A441]">{number}</div><div><h3 className="text-xl font-black">{title}</h3><p className="mt-2 leading-7 text-slate-700">{text}</p></div></div></div>)}</div></div><div className="grid gap-5"><img src="/plan-mockup.svg" alt="Mockup visual de plan de negocio" className="rounded-[2rem] shadow-2xl" /><img src="/dashboard-mockup.svg" alt="Mockup visual de análisis financiero" className="rounded-[2rem] shadow-2xl" /></div></div></section>

      <section id="servicios" className="py-20"><div className="mx-auto max-w-7xl px-6 lg:px-8"><div className="mx-auto max-w-3xl text-center"><SectionTag>{t.servicesTag}</SectionTag><h2 className="text-3xl font-black leading-tight md:text-5xl">{t.servicesTitle}</h2><p className="mt-5 text-lg leading-8 text-slate-300">{t.servicesText}</p></div><div className="mt-12 grid gap-6 lg:grid-cols-3">{t.services.map((service) => <div key={service.title} className="rounded-[2rem] border border-white/10 bg-white/[0.06] p-6 shadow-2xl"><p className="text-sm font-bold uppercase tracking-[0.2em] text-[#D9A441]">{service.time}</p><h3 className="mt-3 text-2xl font-black">{service.title}</h3><p className="mt-3 min-h-[112px] leading-7 text-slate-300">{service.description}</p><div className="mt-5 rounded-2xl bg-[#D9A441]/10 p-4"><p className="text-2xl font-black text-[#D9A441]">{service.cop}</p><p className="mt-1 text-lg font-bold text-white">{service.usd}</p></div><div className="mt-6 space-y-3">{service.items.map((item) => <div key={item} className="flex gap-3"><CheckCircle2 className="mt-1 shrink-0 text-[#D9A441]" size={18} /><p className="text-sm leading-6 text-slate-300">{item}</p></div>)}</div><a href={whatsappUrl} target="_blank" rel="noreferrer" className="mt-7 inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/15 px-6 py-4 font-bold text-white transition hover:bg-white/10">{t.servicesCta} <ArrowRight size={18} /></a></div>)}</div></div></section>

      <section className="bg-[#F6F0E6] py-20 text-[#090B12]"><div className="mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-center lg:px-8"><div><SectionTag>{t.filterTag}</SectionTag><h2 className="text-3xl font-black leading-tight md:text-5xl">{t.filterTitle}</h2><p className="mt-5 text-lg leading-8 text-slate-700">{t.filterText}</p></div><div className="rounded-[2rem] bg-white p-6 shadow-2xl"><div className="space-y-4">{t.questions.map((question, index) => <div key={question} className="flex gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#090B12] font-black text-[#D9A441]">{index + 1}</div><p className="pt-2 font-semibold text-slate-700">{question}</p></div>)}</div><a href={whatsappUrl} target="_blank" rel="noreferrer" className="mt-6 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#090B12] px-6 py-4 font-bold text-white transition hover:scale-[1.01]">{t.answerCta} <MessageCircle size={18} /></a></div></div></section>
      ...
      <section id="contacto" className="relative overflow-hidden bg-[#D9A441] py-20 text-[#090B12]"><div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.45),transparent_30%)]" /><div className="relative mx-auto grid max-w-7xl gap-10 px-6 lg:grid-cols-[1fr_0.85fr] lg:items-center lg:px-8"><div><p className="text-sm font-black uppercase tracking-[0.22em]">{t.contactTag}</p><h2 className="mt-3 text-3xl font-black leading-tight md:text-5xl">{t.contactTitle}</h2><p className="mt-5 max-w-2xl text-lg leading-8 text-[#2A210D]">{t.contactText}</p></div><div className="rounded-[2rem] bg-[#090B12] p-6 text-white shadow-2xl"><div className="flex items-center gap-4"><img src="/gabriel-profile.jpg" alt="Gabriel Castellanos" className="h-16 w-16 rounded-2xl object-cover object-top" /><div><h3 className="text-2xl font-black">{t.contactBoxTitle}</h3><p className="mt-1 text-slate-300">WhatsApp: +971 54 539 7901</p></div></div><div className="mt-6 space-y-3"><a href={whatsappUrl} target="_blank" rel="noreferrer" className="inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#D9A441] px-6 py-4 font-bold text-[#090B12] transition hover:bg-[#F1C45F]">{t.whatsappLabel} <MessageCircle size={18} /></a><a href={LINKEDIN_URL} target="_blank" rel="noreferrer" className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/15 px-6 py-4 font-bold text-white transition hover:bg-white/10">{t.linkedinLabel} <ArrowRight size={18} /></a></div></div></div></section>
